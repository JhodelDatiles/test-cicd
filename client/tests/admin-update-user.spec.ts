...
test.describe('Admin update user', () => {
  test('edits a user from the modal and updates the table', async ({ page }) => {
    await page.route('**/auth/login', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ accessToken: 'mock-token', user: mockAdminUser }),
      });
    });
    ...
    await expect(page.getByTestId(`admin-user-row-${mockUser._id}`)).toBeVisible();
    ...
    await Promise.all([
      page.waitForResponse(
        (res) => res.url().includes(`/crud/users/${mockUser._id}`) && res.request().method() === 'PATCH'
      ),
      page.getByTestId('edit-user-submit').click(),
    ]);

    await expect(page.getByTestId(`admin-user-row-${mockUser._id}`)).toContainText('janet');
  });
  ...
  test('shows an error when the update request fails', async ({ page }) => {
    ...
    await expect(page.getByTestId('edit-user-error')).toHaveText('Email already exists');
  });
});